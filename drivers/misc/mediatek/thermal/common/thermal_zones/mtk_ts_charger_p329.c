...
static int mtktscharger_register_thermal(void)
{
	...
	/* trips : trip 0~2 */
	thz_dev = mtk_thermal_zone_device_register("mtk_ts_charger", trips, num_trip,
...
void mtk_ts_charger_unregister_cooler(void)
{
	if (cl_dev_sysrst) {
...
static int mtktscharger_pdrv_probe(struct platform_device *pdev)
{
	...
	if (!mtktscharger_dir) {
		...
		entry = proc_create("tz_charger", 0664, mtktscharger_dir,
							&mtktscharger_fops);
		if (entry)
			proc_set_user(entry, uid, gid);
		entry = proc_create("tz_charger_param", 0664, mtktscharger_dir,
	...
	return 0;
err_unreg:
	mtk_ts_charger_unregister_cooler();
	return 0;
}
