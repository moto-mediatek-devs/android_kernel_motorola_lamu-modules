...
{
	pr_notice("E_WF: thermal_usb: %s doesn't exist\n", __func__);
	return 0;
}
int __attribute__ ((weak))
...
/* This is to preserve last temperature readings from usb driver.
 * In case mtk_ts_usb.c fails to read temperature.
 */
/**
...
struct BTSUSB_TEMPERATURE {
	__s32 BTSUSB_Temp;
	__s32 TemperatureR;
};
static int g_RAP_pull_up_R = BTSUSB_RAP_PULL_UP_R;
static int g_TAP_over_critical_low = BTSUSB_TAP_OVER_CRITICAL_LOW;
static int g_RAP_pull_up_voltage = BTSUSB_RAP_PULL_UP_VOLTAGE;
static int g_RAP_ntc_table = BTSUSB_RAP_NTC_TABLE;
static int g_RAP_ADC_channel = BTSUSB_RAP_ADC_CHANNEL;
static int g_btsusb_TemperatureR;
/* struct BTSUSB_TEMPERATURE BTSUSB_Temperature_Table[] = {0}; */
static struct BTSUSB_TEMPERATURE *BTSUSB_Temperature_Table;
static int ntc_tbl_size;
/* AP_NTC_BL197 */
static struct BTSUSB_TEMPERATURE BTSUSB_Temperature_Table1[] = {
	{-40, 74354},		/* FIX_ME */
	{-35, 74354},		/* FIX_ME */
	{-30, 74354},		/* FIX_ME */
	{-25, 74354},		/* FIX_ME */
...
/* AP_NTC_TSM_1 */
static struct BTSUSB_TEMPERATURE BTSUSB_Temperature_Table2[] = {
	{-40, 70603},		/* FIX_ME */
	{-35, 70603},		/* FIX_ME */
	{-30, 70603},		/* FIX_ME */
	{-25, 70603},		/* FIX_ME */
...
/* AP_NTC_10_SEN_1 */
static struct BTSUSB_TEMPERATURE BTSUSB_Temperature_Table3[] = {
	{-40, 74354},		/* FIX_ME */
	{-35, 74354},		/* FIX_ME */
	{-30, 74354},		/* FIX_ME */
	{-25, 74354},		/* FIX_ME */
...
static struct BTSUSB_TEMPERATURE BTSUSB_Temperature_Table4[] = {
	{-40, 188500},
	{-35, 144290},
	{-30, 111330},
	{-25, 86560},
...
/* AP_NTC_47 */
static struct BTSUSB_TEMPERATURE BTSUSB_Temperature_Table5[] = {
	{-40, 483954},		/* FIX_ME */
	{-35, 483954},		/* FIX_ME */
	{-30, 483954},		/* FIX_ME */
	{-25, 483954},		/* FIX_ME */
...
/* NTCG104EF104F(100K) */
static struct BTSUSB_TEMPERATURE BTSUSB_Temperature_Table6[] = {
	{-40, 4251000},
	{-35, 3005000},
	{-30, 2149000},
	{-25, 1554000},
...
static struct BTSUSB_TEMPERATURE BTSUSB_Temperature_Table7[] = {
	{-40, 4397119},
	{-35, 3088599},
	{-30, 2197225},
	{-25, 1581881},
...
static __s16 mtkts_btsusb_thermistor_conver_temp(__s32 Res)
{
	int i = 0;
	int asize = 0;
	__s32 RES1 = 0, RES2 = 0;
	__s32 TAP_Value = -200, TMP1 = 0, TMP2 = 0;
	asize = (ntc_tbl_size / sizeof(struct BTSUSB_TEMPERATURE));
	/* mtktsusb_dprintk("btsusb() :
	 * asize = %d, Res = %d\n",asize,Res);
	 */
	if (Res >= BTSUSB_Temperature_Table[0].TemperatureR) {
		TAP_Value = -40;	/* min */
	} else if (Res <=
		BTSUSB_Temperature_Table[asize - 1].TemperatureR) {
		TAP_Value = 125;	/* max */
	} else {
		RES1 = BTSUSB_Temperature_Table[0].TemperatureR;
		TMP1 = BTSUSB_Temperature_Table[0].BTSUSB_Temp;
		/* mtktsusb_dprintk("%d : RES1 = %d,TMP1 = %d\n",__LINE__,
		 * RES1,TMP1);
		 */
		for (i = 0; i < asize; i++) {
			if (Res >=
				BTSUSB_Temperature_Table[i].TemperatureR) {
				RES2 =
				 BTSUSB_Temperature_Table[i].TemperatureR;
				TMP2 =
				BTSUSB_Temperature_Table[i].BTSUSB_Temp;
				/* mtktsusb_dprintk("%d :i=%d, RES2 = %d,
				 * TMP2 = %d\n",__LINE__,i,RES2,TMP2);
				 */
				break;
			}
			RES1 = BTSUSB_Temperature_Table[i].TemperatureR;
			TMP1 = BTSUSB_Temperature_Table[i].BTSUSB_Temp;
...
static __s16 mtk_ts_btsusb_volt_to_temp(__u32 dwVolt)
{
	__s32 TRes;
	__u64 dwVCriAP = 0;
	__u64 dwVCriAP2 = 0;
	__s32 BTSUSB_TMP = -100;
	/* SW workaround-----------------------------------------------------
...
#endif
	output = mtk_ts_btsusb_volt_to_temp(ret);
	mtktsusb_dprintk_always("BTSUSB ret = %d, temperature = %d\n",
								ret, output);
	return output;
}
...
static int mtktsusb_register_thermal(void)
{
	mtktsusb_dprintk("%s\n", __func__);
	/* trips : trip 0~2 */
	thz_dev = mtk_thermal_zone_device_register("mtktsusb", trips, num_trip,
					NULL, /* name: mtktsusb ??? */
...
void mtkts_btsusb_prepare_table(int table_num)
{
	switch (table_num) {
	case 1:		/* AP_NTC_BL197 */
		BTSUSB_Temperature_Table = BTSUSB_Temperature_Table1;
		ntc_tbl_size = sizeof(BTSUSB_Temperature_Table1);
		break;
	case 2:		/* AP_NTC_TSM_1 */
		BTSUSB_Temperature_Table = BTSUSB_Temperature_Table2;
		ntc_tbl_size = sizeof(BTSUSB_Temperature_Table2);
		break;
	case 3:		/* AP_NTC_10_SEN_1 */
		BTSUSB_Temperature_Table = BTSUSB_Temperature_Table3;
		ntc_tbl_size = sizeof(BTSUSB_Temperature_Table3);
		break;
	case 4:		/* AP_NTC_10 */
		BTSUSB_Temperature_Table = BTSUSB_Temperature_Table4;
		ntc_tbl_size = sizeof(BTSUSB_Temperature_Table4);
		break;
	case 5:		/* AP_NTC_47 */
		BTSUSB_Temperature_Table = BTSUSB_Temperature_Table5;
		ntc_tbl_size = sizeof(BTSUSB_Temperature_Table5);
		break;
	case 6:		/* NTCG104EF104F */
		BTSUSB_Temperature_Table = BTSUSB_Temperature_Table6;
		ntc_tbl_size = sizeof(BTSUSB_Temperature_Table6);
		break;
	case 7:		/* NCP15WF104F03RC */
		BTSUSB_Temperature_Table = BTSUSB_Temperature_Table7;
		ntc_tbl_size = sizeof(BTSUSB_Temperature_Table7);
		break;
	default:		/* AP_NTC_10 */
		BTSUSB_Temperature_Table = BTSUSB_Temperature_Table4;
		ntc_tbl_size = sizeof(BTSUSB_Temperature_Table4);
	...
	pr_notice("[Thermal/TZ/BTSUSB] %s table_num=%d\n",
						__func__, table_num);
}
...
static ssize_t mtkts_btsusb_param_write(
struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
	int len = 0;
	struct mtktsbtsusb_param_data {
		char desc[512];
	...
	struct mtktsbtsusb_param_data *ptr_mtktsbtsusb_parm_data;
	ptr_mtktsbtsusb_parm_data = kmalloc(
				sizeof(*ptr_mtktsbtsusb_parm_data),
					GFP_KERNEL);
	if (ptr_mtktsbtsusb_parm_data == NULL)
		return -ENOMEM;
	 ...
	 */
	ptr_mtktsbtsusb_parm_data->adc_channel = 11;
	len = (count < (sizeof(ptr_mtktsbtsusb_parm_data->desc) - 1)) ?
			count :
			(sizeof(ptr_mtktsbtsusb_parm_data->desc) - 1);
	if (copy_from_user(ptr_mtktsbtsusb_parm_data->desc, buffer, len)) {
		kfree(ptr_mtktsbtsusb_parm_data);
		return 0;
	}
	ptr_mtktsbtsusb_parm_data->desc[len] = '\0';
	mtktsusb_dprintk("[%s]\n", __func__);
	if (sscanf
	    (ptr_mtktsbtsusb_parm_data->desc,
		"%9s %d %9s %d %15s %d %9s %d %d",
		ptr_mtktsbtsusb_parm_data->pull_R,
		&ptr_mtktsbtsusb_parm_data->valR,
		ptr_mtktsbtsusb_parm_data->pull_V,
		&ptr_mtktsbtsusb_parm_data->valV,
		ptr_mtktsbtsusb_parm_data->overcrilow,
		&ptr_mtktsbtsusb_parm_data->over_cri_low,
		ptr_mtktsbtsusb_parm_data->NTC_TABLE,
		&ptr_mtktsbtsusb_parm_data->ntc_table,
		&ptr_mtktsbtsusb_parm_data->adc_channel) >= 8) {
		if (!strcmp(ptr_mtktsbtsusb_parm_data->pull_R, "PUP_R")) {
			g_RAP_pull_up_R = ptr_mtktsbtsusb_parm_data->valR;
		...
		if (!strcmp(ptr_mtktsbtsusb_parm_data->pull_V,
			"PUP_VOLT")) {
		...
		if (!strcmp(ptr_mtktsbtsusb_parm_data->overcrilow,
			"OVER_CRITICAL_L")) {
		...
		if (!strcmp(ptr_mtktsbtsusb_parm_data->NTC_TABLE,
			"NTC_TABLE")) {
			g_RAP_ntc_table =
				ptr_mtktsbtsusb_parm_data->ntc_table;
		 ...
		 * choose "adc_channel=11" to check if there is any param input
		 */
		if ((ptr_mtktsbtsusb_parm_data->adc_channel >= 2)
		&& (ptr_mtktsbtsusb_parm_data->adc_channel <= 11))
			 ...
			 */
			if (ptr_mtktsbtsusb_parm_data->adc_channel != 11)
				g_RAP_ADC_channel =
				ptr_mtktsbtsusb_parm_data->adc_channel;
			else
				g_RAP_ADC_channel = AUX_IN2_NTC;
		}
		...
		mtktsusb_dprintk("g_RAP_ADC_channel=%d\n",
						g_RAP_ADC_channel);
		mtkts_btsusb_prepare_table(g_RAP_ntc_table);
		kfree(ptr_mtktsbtsusb_parm_data);
		return count;
	}
	mtktsusb_dprintk("[%s] bad argument\n", __func__);
...
static int mtkts_btsusb_param_read(struct seq_file *m, void *v)
{
	seq_printf(m, "%d\n", g_RAP_pull_up_R);
	seq_printf(m, "%d\n", g_RAP_pull_up_voltage);
	seq_printf(m, "%d\n", g_TAP_over_critical_low);
...
static int mtkts_btsusb_param_open(struct inode *inode, struct file *file)
{
	return single_open(file, mtkts_btsusb_param_read, NULL);
}
static const struct proc_ops mtktsusb_fops = {
	.proc_open = mtktsusb_open,
	.proc_read = seq_read,
	.proc_lseek = seq_lseek,
	.proc_write = mtktsusb_write,
...
static const struct proc_ops mtkts_btsusb_param_fops = {
	.proc_open = mtkts_btsusb_param_open,
	.proc_read = seq_read,
	.proc_lseek = seq_lseek,
	.proc_write = mtkts_btsusb_param_write,
	.proc_release = single_release,
};
#if IS_ENABLED(CONFIG_DEVICE_MODULES_MEDIATEK_MT6577_AUXADC)
