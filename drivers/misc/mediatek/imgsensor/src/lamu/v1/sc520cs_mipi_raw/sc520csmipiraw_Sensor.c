...
static struct imgsensor_info_struct imgsensor_info = {
	...
	.pre = {
		...
		.max_framerate = 300,
		.mipi_pixel_rate = 180000000, //(912M*2/10)
	},
	.cap = {
		.pclk = 90000000, .linelength = 1500, 
		.framelength = 2000, 
		.startx = 0, 
		.starty = 0, 
		.grabwindow_width = 2592, 
	...
	/*  hs04 code for SR-AL6398A-01-12 by liluling at 2022/8/3 start */
...
static struct imgsensor_struct imgsensor = {
	...
	.shutter = 0x3d0, // current shutter   // Danbo ?? //0xf97
	.gain = 0x40,	  // current gain     // Danbo ?? //0x80
	.dummy_pixel = 0,
	.dummy_line = 0,
...
static void set_shutter_frame_length(kal_uint32 shutter,kal_uint16 frame_length)
{
	...
	CAM_DBG(PFX,"Add for DUAL shutter =%d, framelength =%d,imgsensor.autoflicker_en=%d,realtime_fps=%d\n", shutter, imgsensor.frame_length,imgsensor.autoflicker_en,realtime_fps);
}
...
{
	kal_uint16 reg_gain;
	kal_uint32 d_gain,dgain_r,dgain_g,dgain_b;
	kal_uint32 dgain_r_coser,dgain_r_fine,dgain_g_coser,dgain_g_fine,dgain_b_coser,dgain_b_fine;
	...
	d_gain = reg_gain * SC520CS_SENSOR_GAIN_BASE / SC520CS_AGC_Param[gain_index][0]/8;
	
	dgain_r=d_gain*wb_rgain/128;
	dgain_g=d_gain*wb_ggain/128;
	dgain_b=d_gain*wb_bgain/128;
	
	if(dgain_r<256){
	dgain_r_coser=0x00;
	dgain_r_fine=dgain_r;
	}
	else{
	dgain_r_coser=0x01;
	dgain_r_fine=dgain_r/2;	
	}
	if(dgain_g<256){
	dgain_g_coser=0x00;
	dgain_g_fine=dgain_g;
	}
	else{
	dgain_g_coser=0x01;
	dgain_g_fine=dgain_g/2;	
	}	
	if(dgain_b<256){
	dgain_b_coser=0x00;
	dgain_b_fine=dgain_b;
	}
	else{
	dgain_b_coser=0x01;
	dgain_b_fine=dgain_b/2;	
	}
	...
	write_cmos_sensor(0x3e67, dgain_r_fine  & 0xff);
	write_cmos_sensor(0x3e68, dgain_b_coser & 0xff);
...
{
	CAM_DBG(PFX,"image_mirror = %d", image_mirror);
...
static void sensor_init(void)
{
	...
	write_cmos_sensor8(0x301f,0x43);
...
/* hs04 code for DEAL6398A-935 by pengxutao at 2022/08/18 end */
#if MULTI_WRITE
...
static void capture_setting(kal_uint16 currefps)
{
	...
	if (currefps == 300)
	{
		CAM_DBG(PFX,"capture_setting fps = 300\n");
	...
	{
		CAM_DBG(PFX,"capture_setting fps not 300\n");
		//  write_cmos_sensor8(0x0100,0x01);
		write_cmos_sensor8(0x3221, 0x00);
		mDELAY(10);
	}
...
{
	CAM_DBG(PFX,"slim_video_setting\n");
#if MULTI_WRITE
	sc520cs_table_write_cmos_sensor(
...
static int read_sc520cs_awb_info(kal_uint32 addr)
{
	...
	int  rg = 128, bg = 128, golden_rg = 128, golden_bg = 128;
	int base_gain=1024;
	int r_ratio, b_ratio, g_ratio;
	...
	if (check_sum_awb == check_sum_awb_cal) {
		...
		wb_bgain = GAIN_DEFAULT * b_ratio / base_gain;
		
		wb_rgain = (wb_rgain>256)?128:wb_rgain;
		wb_ggain = (wb_ggain>256)?128:wb_ggain;
...
static kal_uint32 open(void)
{
	...
	CAM_DBG(PFX,"[open]: PLATFORM:MT6765,MIPI 2LANE\n");
	CAM_DBG(PFX,"preview  2592*1944@30fps,912Mbp/lane;"
			"capture 2592*1944@30fps,912Mbps/lane\n");
	while (imgsensor_info.i2c_addr_table[i] != 0xff)
	{
		spin_lock(&imgsensor_drv_lock);
		imgsensor.i2c_write_id = imgsensor_info.i2c_addr_table[i];
	...
	case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
		...
		else
		{
			if (imgsensor.current_fps !=
				...
				CAM_DBG(PFX,"fps %d fps not support,use cap: %d fps!\n",
	...
	UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_MIPI_PIXEL_RATE:
	{
		...
		CAM_DBG(PFX,"sc500sc SENSOR_FEATURE_GET_MIPI_PIXEL_RATE");
