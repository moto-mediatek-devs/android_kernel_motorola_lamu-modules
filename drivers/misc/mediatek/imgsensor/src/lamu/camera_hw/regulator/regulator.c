	...
	if (pregulator) {
		if (pin_state != IMGSENSOR_HW_PIN_STATE_LEVEL_0) {
			...
			if (regulator_set_voltage(
				...
				if (pin == IMGSENSOR_HW_PIN_DVDD && voltage == REGULATOR_VOLTAGE_1000
				&& sensor_idx == 0)/*Determining which sensor requires changing the voltage*/ {
				pr_info("set_sub_sensor vcamd to %dmV\n",
						voltage/1000);
				regmap_write(g_regmap, MT6358_VCAMD_ANA_CON0,0x405);
				//From the datasheet, AVDD needs to be set to increase the offset voltage by 50mA
