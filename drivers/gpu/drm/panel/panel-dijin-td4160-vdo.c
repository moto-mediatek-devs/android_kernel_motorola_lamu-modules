...
#if 0
static ssize_t dijin_disp_set_dre_read(struct file *filp, char __user *buff, size_t size, loff_t *pos)
{
	u32 len = 0;
...
static ssize_t dijin_disp_set_dre_write(struct file *filp, const char *buff, size_t size, loff_t *pos)
{
	char cmd[16] = { 0 };
	ssize_t ret;
...
static struct proc_ops proc_dijin_dre_fops = {
	.proc_read = dijin_disp_set_dre_read,
	.proc_write = dijin_disp_set_dre_write,
	.proc_lseek = default_llseek,
};

dijin_proc_node lcd_info_proc[] = {
	{"backlight_hbm", NULL, &proc_dijin_hbm_fops, false},
	{"disp_set_dre", NULL, &proc_dijin_dre_fops, false},
};
#endif
