...
#include "../../drivers/misc/mediatek/typec/tcpc/inc/tcpci_timer.h"
...
#define HUSB320_DRP_ACC                (HUSB320_DRP | HUSB320_AUD_ACC)
#define HUSB320_SNK_ACC                (HUSB320_SNK | HUSB320_AUD_ACC)
#define HUSB320_SRC_ACC                (HUSB320_SRC | HUSB320_AUD_ACC)
...
#define HUSB320_TYPE_SRC_ACC           (HUSB320_TYPE_SRC | HUSB320_TYPE_ACTV_CABLE)
...
static bool platform_get_device_irq_state(struct husb320_chip *chip)
{
	struct device *cdev = &chip->client->dev;
	int rc = 0;
...
#else
		if (gpiod_cansleep(chip->pdata->int_gpio)) {
...
static void husb320_orient_notice(struct husb320_chip *chip)
{
	struct device *cdev = &chip->client->dev;
	int rc;
	u8 status;
...
static int husb320_set_msg_header(struct tcpc_device *tcpc,
					uint8_t power_role, uint8_t data_role)
{
	pr_err("pzl husb320_set_rx_enable  enter\n");
	return 0;
}
...
#if IS_ENABLED(CONFIG_OEM_DEVINFO)
	FULL_PRODUCT_DEVICE_INFO(ID_CC_LOGIC, "HUSB320");
#endif
