...
#elif (LINUX_VERSION_CODE < KERNEL_VERSION(5, 15, 999))
...
#elif (LINUX_VERSION_CODE < KERNEL_VERSION(4, 19, 999))
...
static int __sc8989x_get_adc(struct sc8989x_chip *sc, enum sc8989x_adc_channel chan, int *val)
{
	int reg_val, ret = 0;
...
static int sc8989x_get_vbus_voltage(struct sc8989x_chip *sc)
{
	int ret = 0;
	int vbus = 0;
	int value = 0;
 ...
 */
static int sc8989x_get_property(struct charger_device *chg_dev,
			enum charger_property prop, union charger_propval *val)
{
	...
	dev_info(sc->dev, "%s: prop:%d \n", __func__, prop);

	switch (prop) {
#if IS_ENABLED(CONFIG_OEM_TINNO_CHARGER)
	case CHARGER_PROP_CHARGER_ENABLED:
		ret = sc8989x_check_chg_enabled(sc, &is_enabled);
		if (!ret) {
	...
	case CHARGER_PROP_CHARGER_VOLTAGE:
		val->intval = sc8989x_get_vbus_voltage(sc);;
		break;
	...
	case CHARGER_PROP_CHARGER_TYPE:
		...
		break;
	case CHARGER_PROP_VBAT:
		ret = __sc8989x_get_adc(sc, SC8989X_ADC_VBAT, &value);
		if (!ret) {
			val->intval = value * 1000;
		}
		break;
	case CHARGER_PROP_IBAT:
		ret = __sc8989x_get_adc(sc, SC8989X_ADC_ICC, &value);
		if (!ret) {
			val->intval = value * 1000;
		}
		break;
	case CHARGER_PROP_VBUS:
		ret = __sc8989x_get_adc(sc, SC8989X_ADC_VBUS, &value);
		if (!ret) {
			val->intval = value * 1000;
		}
		break;
	case CHARGER_PROP_IBUS:
		ret = __sc8989x_get_adc(sc, SC8989X_ADC_IBUS, &value);
		if (!ret) {
			val->intval = value * 1000;
		}
		break;
...
static struct charger_ops sc8989x_chg_ops = {
	...
	/* get/set property*/
	.get_property = sc8989x_get_property,
};
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 15, 0))
static int charger_detect_init(struct sc8989x_chip *sc)
{
	struct phy *phy;
	int ret;
...
static int charger_detect_release(struct sc8989x_chip *sc)
{
	struct phy *phy;
	int ret;
...
/* TN Begin modified by rongxing.li/860655 20231125 CR/EKFOGO4G-5119 */
#if IS_ENABLED(CONFIG_FACTORY_BUILD)
	dev_info(sc->dev, "%s disable charging for factory version\n", __func__);
	boot_init = true;
	ret = sc8989x_set_chg_enable(sc, false);
	if (ret)
		dev_err(sc->dev, "%s disable charging failed\n", __func__);
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(6, 6, 0))
static void sc8989x_charger_remove(struct i2c_client *client)
#else
