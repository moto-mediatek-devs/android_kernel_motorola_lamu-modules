...
static int set_clks(bool enable)
{
	...
	fpsensor_log(INFO_LOG, "fpc %s entry. status:%d \n", __func__, enable);
...
static ssize_t clk_enable_get(struct device *device,
		struct device_attribute *attr, char *buffer)
{
	fpsensor_log(INFO_LOG, "fpc %s entry clk_status:%d\n", __func__, g_fpsensor_clocks_enabled);
	return scnprintf(buffer, PAGE_SIZE, "%i\n", g_fpsensor_clocks_enabled);
}
...
static DEVICE_ATTR(clk_enable, S_IRUSR | S_IWUSR, clk_enable_get, clk_enable_set);
...
static int mtk6797_probe(struct spi_device *spidev)
{
	...
	int irq_num = 0;
#endif
	printk("iker fpc mtk6797_probe start\n");
	fpsensor_log(INFO_LOG, "fpc %s entry.\n", __func__);
	...
	fpsensor_log(INFO_LOG, "%s: ok\n", __func__);
	printk("iker fpc mtk6797_probe end\n");
    func_exit();
	return rc;
#endif 
...
void mtk6797_remove(struct spi_device *spidev)
{
