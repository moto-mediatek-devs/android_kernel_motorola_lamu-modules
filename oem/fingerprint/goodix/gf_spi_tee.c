...
extern struct spi_device *g_fpsensor_spidev;
...
static long gf_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	*/
	...
	case GF_IOC_INIT:
		...
		ret = mtk_disp_notifier_register("gf_drm_notify", &gf_dev->notifier);
		if (ret < 0)
			gf_debug(ERR_LOG, "[DRM] gf mtk_disp_notifier_register fail: %d\n", ret);
#else
