...
int nvt_gesture_mode = 0;
EXPORT_SYMBOL(nvt_gesture_mode);
#endif

void (*lcd_nvt_resume_nt36528a)(void);
...
static void tianma_panel_init(struct tianma *ctx)
{
	...
	tianma_dcs_write_seq_static(ctx, 0x68,0x04,0x01);
...
static struct proc_dir_entry *proc_dir_tianma_lcd_info;
...
#if 1
static ssize_t tianma_extra_brightness_read(struct file *filp, char __user *buff, size_t size, loff_t *pos)
{
	u32 len = 0;
...
static ssize_t tianma_extra_brightness_write(struct file *filp, const char *buff, size_t size, loff_t *pos)
{
	char cmd[16] = { 0 };
	ssize_t ret;
...
static struct proc_ops proc_tianma_extra_fops = {
	.proc_read = tianma_extra_brightness_read,
	.proc_write = tianma_extra_brightness_write,
	.proc_lseek = default_llseek,
};

tianma_proc_node lcd_info_proc[] = {
	{"backlight_hbm", NULL, &proc_tianma_hbm_fops, false},
	{"extra_brightness", NULL, &proc_tianma_extra_fops, false},
};
#endif
...
static int tianma_probe(struct mipi_dsi_device *dsi)
{
	...
	proc_dir_tianma_lcd_info = proc_mkdir("lcd_info", NULL);
	for (; i < ARRAY_SIZE(lcd_info_proc); i++) {
		lcd_info_proc[i].node = proc_create(lcd_info_proc[i].name, 0644,
					proc_dir_tianma_lcd_info, lcd_info_proc[i].fops);
...
#if IS_ENABLED(CONFIG_OEM_DEVINFO)
	FULL_PRODUCT_DEVICE_INFO(ID_LCD, "TIANMA-NT36528A-VDO");
#endif
...
#if 1
static void tianma_shutdown(struct mipi_dsi_device *dsi)
{
	struct tianma *ctx = mipi_dsi_get_drvdata(dsi);
...
static const struct of_device_id tianma_of_match[] = {
	{ .compatible = "tianma,nt36528a,vdo", },
	{ }
};
...
static struct mipi_dsi_driver tianma_driver = {
	...
	.driver = {
		.name = "nt36528a_dsi_vdo_tianma",
		.owner = THIS_MODULE,
		.of_match_table = tianma_of_match,
	},
	.shutdown = tianma_shutdown,
};
...
MODULE_DESCRIPTION("tianma nt36528a VDO LCD Panel Driver");
