...
static inline struct txd *panel_to_lcm(struct drm_panel *panel)
{
...
static int txd_panel_bias_regulator_init(void)
{
...
static int txd_panel_bias_enable(void)
{
	int ret = 0;
	int retval = 0;

	txd_panel_bias_regulator_init();
...
static int txd_panel_bias_disable(void)
{
	int ret = 0;
	int retval = 0;
...
{
	struct txd *ctx = panel_to_lcm(panel);
	...
	void *handle, unsigned int level)
{
	...
	bl_lvl = backlight_i2c_map4[level];
...
static struct proc_dir_entry *proc_dir_txd_lcd_info;
...
static ssize_t txd_hbm_read(struct file *filp, char __user *buff, size_t size, loff_t *pos)
{
	u32 len = 0;
...
static ssize_t txd_hbm_write(struct file *filp, const char *buff, size_t size, loff_t *pos)
{
...
static struct proc_ops proc_txd_hbm_fops = {
	.proc_read = txd_hbm_read,
	.proc_write = txd_hbm_write,
...
MODULE_DESCRIPTION("txd ili9883c VDO LCD Panel Driver");
