...
__maybe_unused static int sc2150_private_init(struct i2c_client *client) {
	...
	u8 ldo_trim = 0xBB;
	u8 tx_pd_data = 0x26;
	u8 continuous_data = 0x78;

	sc2150_read_device(client, 0xFF, 1, &data);
	if (!(data & 0x80)) {
		for (i = 0; i < 6; i++) {
			sc2150_write_device(client, 0xFF, 1, &pass_word[i]);
		}
		sc2150_read_device(client, 0xFF, 1, &data);
		if (!(data & 0x80)) {
			dev_err(&client->dev, "enter test mode fail\n");
			return -EINVAL;
		}
	}
	sc2150_write_device(client, 0xC3, 1, &ldo_trim);
	sc2150_write_device(client, 0xC4, 1, &tx_pd_data);
	sc2150_write_device(client, 0xE7, 1, &continuous_data);
