...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
static int nu2115_get_property(struct charger_device *chg_dev,
		enum charger_property prop, union charger_propval *val)
{
	...
	dev_info(chip->dev, "%s: prop:%d \n", __func__, prop);

	switch (prop) {
#if IS_ENABLED(CONFIG_TINNO_CHARGER)
	case CHARGER_PROP_CHARGER_ENABLED:
		ret = nu2115_is_charger_enabled(chip, &is_enabled);
		if (ret < 0) {
			dev_err(chip->dev, "%s: get charge enable status failed(%d)\n", __func__, ret);
		} else {
			val->intval = (int)is_enabled;
		}
		break;
#endif
	default:
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
	.get_property = nu2115_get_property,
#endif
