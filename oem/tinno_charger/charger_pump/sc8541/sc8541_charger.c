...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
static int mtk_sc8541_get_property(struct charger_device *chg_dev,
		enum charger_property prop, union charger_propval *val)
{
	...
	case CHARGER_PROP_CHARGER_ENABLED:
		ret = sc8541_check_charge_enabled(sc, &is_enabled);
		if (ret < 0) {
			dev_err(sc->dev, "%s: get charge enable status failed(%d)\n", __func__, ret);
		} else {
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 10, 0))
	.get_property = mtk_sc8541_get_property,
#endif
