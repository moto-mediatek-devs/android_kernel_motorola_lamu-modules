...
static uint8_t g_prot_update_fw_flag;
...
static uint32_t get_pack_checksum(uint8_t *data, uint16_t length, uint8_t module, uint8_t command)
{
	...
	for (i = 0; i < length; i += 4) {
		check_sum += AW_SAR_GET_32_DATA(data[i + 0], data[i + 1], data[i + 2], data[i + 3]);
	}
...
static int32_t
soc_to_dri_pack_parse(AW_SAR_I2C_TYPE i2c, uint32_t length, uint8_t module, uint8_t command)
{
	int32_t ret = -1;
...
static uint32_t
aw_get_bin_checksum(const uint8_t *w_bin_offset, uint32_t update_data_len, uint32_t check_len)
{
	uint32_t i = 0;
	uint32_t check_sum = 0;
	uint32_t tmp = 0;
							...
							uint8_t *send_value, uint16_t send_val_len)
{
	...
	//1.send cmd
	ret = dri_to_soc_pack_send(i2c, module, command, send_val_len, send_value);
	if (ret != AW_FW_OK) {
...
static int32_t aw_send_all_update_cmd(AW_SAR_I2C_TYPE i2c, struct aw_update_fw *update_fw)
{
	...
	do {
		...
		ret = aw_cycle_write_firmware(i2c, fw_data, fw_data_len,
							flash_update_info->tr_start_addr,
...
static int32_t
aw_reg_write_val_to_flash(AW_SAR_I2C_TYPE i2c, uint8_t update_flag, uint16_t addr, uint32_t val)
{
	int32_t ret = 0;
				...
				const uint8_t *w_bin_offset, uint32_t update_data_len)
{
	...
	for (i = 0; i < read_cnt; i += WORD_LEN) {
		ret = aw_reg_read_val(i2c, &r_data, update_info->flash_info->tr_start_addr + i);
		if (ret != AW_FW_OK) {
